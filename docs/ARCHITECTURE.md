...
**2026-09-17 更新：**下文是 2026-09-15 的历史架构快照。二维码签到现改为单进程全局队列：`POST /api/cx/accounts/:uid/sign_by_qrcode` 只返回任务状态，`GET /api/cx/qr-events` 通过 SSE 向同一网站账号的已打开弹窗发送队列状态和最终结果。服务端以学习通 uid 占位并每隔至少 200ms 启动下一任务；`CxAccount.userId` 在扫码提交时校验，重复绑定不再转移归属。详见 [当前二维码接口说明](FRONTEND_API.md)。
